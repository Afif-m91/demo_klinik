...
class Booking extends Admin_Controller {
	...
	public function index()
	{
		...
		$data['title'] = "DATA BOOKING PEMERIKSAAN";
		$data['layout'] = "booking/index";
		...
		list($data['data'],$total) = $this->booking_model->getAll($filter,$limit,$offset,$orderBy,$orderType);
		
		$this->load->library('pagination');
		$config['base_url'] = site_url("booking?");
	...
	public function manage($id = "")
	{
		...
		$data['layout'] = "booking/manage";

		$data['data'] = new StdClass();
		$data['data']->id_pemeriksaan = "";
		$data['data']->id_pasien = "";
		$data['data']->nama_pasien = "";
		$data['data']->keluhan = "";
		$data['data']->tgl_booking = "";
		...
		if($id)
		{
			$dt =  $this->booking_model->get_by("pm.id_pemeriksaan",$id,true);
			if(!empty($dt))
				$data['data'] = $dt;
		}
	...
	public function save()
	{
		...
		if($post)
		{
			...
			if(empty($error))
			{
				if(empty($id))
				{
					$cekpemeriksaan = $this->booking_model->get_by("pm.id_pemeriksaan",$post['id_pemeriksaan']);
					if(!empty($cekpemeriksaan))
			...
			if(empty($error))
			{
				$save = $this->booking_model->save($id,$data,false);
				...
				// if(!empty($id))
				// {
				// 	$this->booking_model->remove_detail($id);
				// }
				...
				foreach($datailkode as $key => $val)
				{
					...
					$detail['id_pasien'] = $val;
					$detail['qty'] = $datailjumlah[$val];
					$this->booking_model->save_detail($detail);
				}
	...
	public function delete($id = "")
	{
		...
		if(!empty($id))
		{
			$cek = $this->booking_model->get_by("pm.id_pemeriksaan",$id,true);
			if(empty($cek))
			{
	...
	public function generate_code()
	{
		...
		$last = $this->booking_model->get_last();
		if(!empty($last))
		{
			$number = substr($last->id_pemeriksaan,10,3) +1;
	...
	public  function cetak($id)
	{
		...
		$data['title'] = "CETAK BOOKING";
		$data['layout'] = "booking/cetak";
		...
		if($id)
		{
			...
			if($dt)
			{
				$this->qrcodeci->generate($dt->id_pemeriksaan);
				$data['data'] = $dt;
	...
	public function rekap()
	{
		...
		$data['title'] = "Laporan Booking Barang";
		$data['layout'] = "booking/rekap";
		...
		list($data['data'],$total) = $this->booking_model->getAll($filter,0,0,"pm.id_pemeriksaan","desc");
	...
	public function export($action,$data,$filter)
	{
		...
		$title = "Laporan Data Booking Barang";
		...
		if(empty($data))
		{
			...
			redirect("booking/rekap?from=".$filter->from."&to=".$filter->to."&status=".$filter->status."");
		}
		else
		{	
			if($action == "excel")
