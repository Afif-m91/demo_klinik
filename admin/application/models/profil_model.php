...
class Profil_Model extends CI_Model
{
	  	...
	  	if (isset($filter))
	  	{
			if (!empty($filter->keyword))
			{
				  $cond[] = "(lower(".$this->key.") like '%" . $this->db->escape_str(strtolower($filter->keyword)) . "%'
								 or lower(judul_profil) like '%" . $this->db->escape_str(strtolower($filter->keyword)) . "%'
								 or lower(isi_profil) like '%" . $this->db->escape_str(strtolower($filter->keyword)) . "%'
								)";
			}
	...
	{
		$where = "WHERE ".$this->key." <> '".$this->db->escape_str(strtolower($id))."' and judul_profil = '".$this->db->escape_str(strtolower($name))."' ";
		$query = $this->db->query("SELECT  *
								   FROM ".$this->table." 
								   $where 
								   ");
	...
	{
		$query = $this->db->query(" ( SELECT  ".$this->key." FROM profil where ".$this->key." = '".$this->db->escape_str(strtolower($id))."' ) ");
		$result = $query->row();
		$query->free_result();
