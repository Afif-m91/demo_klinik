...
class Pengiriman_Model extends CI_Model
{
    ...
    public function getAll($filter = null, $limit = 20, $offset = 0, $orderBy, $orderType)
    {
        ...
        if (isset($filter)) {
            if (!empty($filter->keyword)) {
                $cond[] = "(lower(".$this->key.") like '%" . $this->db->escape_str(strtolower($filter->keyword)) . "%'
								 ...
								 or lower(k.nama_kategori) like '%" . $this->db->escape_str(strtolower($filter->keyword)) . "%'
        ...
        $query = $this->db->query("SELECT
                    SQL_CALC_FOUND_ROWS pg.*,
                    k.nama_kategori, k.keterangan kategori_keterangan,
                    kr.nama kurir,p.nama pelanggan,p.alamat,
                    GROUP_CONCAT(CONCAT(
                        dp.id_barang, '|',
                        b.nama, '|',
                        k.nama_kategori, '|',
                    ...
                    ORDER BY b.nama SEPARATOR '===') AS barang
                    FROM pengiriman pg
    ...
    public function get_by($field, $value = "", $obj = false)
    {
        if (!$field) {
         ...
         // Pengambilan data edit oleh database

        $param = array($this->db->escape_str(strtoupper($value)));
        $query = $this->db->query("SELECT
                        pg.*,
                        k.nama_kategori,
                        k.keterangan kategori_keterangan,
                        kr.nama kurir,
                        p.nama pelanggan,
                        p.alamat,
                        GROUP_CONCAT(CONCAT(
                    ...
                    LEFT JOIN barang b USING(id_barang)
                    LEFT JOIN kategori k USING(id_kategori)
                    LEFT JOIN kurir kr ON kr.id_kurir = pg.id_kurir
                    LEFT JOIN pelanggan p ON p.id_pelanggan = pg.id_pelanggan
                    WHERE $field = ? GROUP BY pg.id_pengiriman", $param);
