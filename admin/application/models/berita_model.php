...
class Berita_Model extends CI_Model
{
	  	...
	  	if (isset($filter))
	  	{
			if (!empty($filter->keyword))
			{
				  $cond[] = "(lower(".$this->key.") like '%" . $this->db->escape_str(strtolower($filter->keyword)) . "%'
								 or lower(judul_berita) like '%" . $this->db->escape_str(strtolower($filter->keyword)) . "%'
								 or lower(isi_berita) like '%" . $this->db->escape_str(strtolower($filter->keyword)) . "%'
								)";
			}
	...
	{
		$where = "WHERE ".$this->key." <> '".$this->db->escape_str(strtolower($id))."' and judul_berita = '".$this->db->escape_str(strtolower($name))."' ";
		$query = $this->db->query("SELECT  *
								   FROM ".$this->table." 
								   $where 
								   ");
	...
	{
		$query = $this->db->query(" ( SELECT  ".$this->key." FROM berita where ".$this->key." = '".$this->db->escape_str(strtolower($id))."' ) ");
		$result = $query->row();
		$query->free_result();
