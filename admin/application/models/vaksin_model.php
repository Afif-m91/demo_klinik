...
class Vaksin_Model extends CI_Model
{
	  	...
	  	if (isset($filter))
	  	{
			if (!empty($filter->keyword))
			{
				  $cond[] = "(lower(".$this->key.") like '%" . $this->db->escape_str(strtolower($filter->keyword)) . "%'
								 or lower(nama_vaksin) like '%" . $this->db->escape_str(strtolower($filter->keyword)) . "%'
								 or lower(usia) like '%" . $this->db->escape_str(strtolower($filter->keyword)) . "%'
								 or lower(kegunaan) like '%" . $this->db->escape_str(strtolower($filter->keyword)) . "%'
								)";
			}
	...
	{
		$where = "WHERE ".$this->key." <> '".$this->db->escape_str(strtolower($id))."' and nama_vaksin = '".$this->db->escape_str(strtolower($name))."' ";
		$query = $this->db->query("SELECT  *
								   FROM ".$this->table." 
								   $where 
								   ");
	...
	{
		$query = $this->db->query(" ( SELECT  ".$this->key." FROM vaksin where ".$this->key." = '".$this->db->escape_str(strtolower($id))."' ) ");
		$result = $query->row();
		$query->free_result();
