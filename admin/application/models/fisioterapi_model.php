...
{
	var $table  = 'fisioterapi';
	var $key  = 'id_fisioterapi';
	  	...
	  	if (isset($filter))
	  	{
			if (!empty($filter->keyword))
			{
				  $cond[] = "(lower(".$this->key.") like '%" . $this->db->escape_str(strtolower($filter->keyword)) . "%'
								 or lower(nama_fisioterapi) like '%" . $this->db->escape_str(strtolower($filter->keyword)) . "%'
								 or lower(tarif) like '%" . $this->db->escape_str(strtolower($filter->keyword)) . "%'
								)";
			}
	...
	{
		$where = "WHERE ".$this->key." <> '".$this->db->escape_str(strtolower($id))."' and nama_fisioterapi = '".$this->db->escape_str(strtolower($name))."' ";
		$query = $this->db->query("SELECT  *
								   FROM ".$this->table." 
								   $where 
								   ");
	...
	{
		$query = $this->db->query(" ( SELECT  ".$this->key." FROM fisioterapi where ".$this->key." = '".$this->db->escape_str(strtolower($id))."' ) ");
		$result = $query->row();
		$query->free_result();
