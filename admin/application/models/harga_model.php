	  	...
	  	if (isset($filter))
	  	{
			if (!empty($filter->keyword))
			{
				  $cond[] = "(lower(".$this->key.") like '%" . $this->db->escape_str(strtolower($filter->keyword)) . "%'
								 ...
								 or lower(harga) like '%" . $this->db->escape_str(strtolower($filter->keyword)) . "%'
								 or lower(moda) like '%" . $this->db->escape_str(strtolower($filter->keyword)) . "%'
								)";
			}
...
// Listing
	...
	{
		$query = $this->db->query(" ( SELECT  ".$this->key." FROM harga where ".$this->key." = '".$this->db->escape_str(strtolower($id))."' ) ");
		$result = $query->row();
		$query->free_result();
	...
	function authenticate($username,$password)
    {
		...
		$this->db->where('id_harga', $this->db->escape_str($username));
