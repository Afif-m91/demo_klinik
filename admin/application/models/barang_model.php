...
class Barang_Model extends CI_Model
{
  ...
  function getRecordset($qry, $qryParam = null)
  {
	...
	function getAll($filter = null,$limit = 20,$offset = 0, $orderBy, $orderType)
	{
	  	...
	  	if (isset($filter))
	  	{
			if (!empty($filter->keyword))
			{
				  $cond[] = "(lower(".$this->key.") like '%" . $this->db->escape_str(strtolower($filter->keyword)) . "%'
								 ...
								 or lower(b.tgl_barang) like '%" . $this->db->escape_str(strtolower($filter->keyword)) . "%'
		...
		$query = $this->db->query("SELECT SQL_CALC_FOUND_ROWS b.*,k.nama_kategori kategori,k.keterangan
								   FROM ".$this->table." b
  ...
  function getData($params = null)
  {
    ...
    if ($params) {
      ...
      if ($params['id_barang'] != 'all') {
        $cat_str = "id_barang = '$params[id_barang]' AND ";
      }
  ...
  function getItemId()
  {
    $query = "SELECT id_barang FROM ".$this->table."";
    $result = $this->getRecordset($query);
    return $result;
  }
	...
	public function get_by($field, $value = "",$obj = false)
	{
		...
		$query = $this->db->query("SELECT b.*,k.nama_kategori kategori,k.keterangan
								   FROM ".$this->table." b
								   LEFT JOIN kategori k on k.id_kategori = b.id_kategori
								   $where
								   ");
